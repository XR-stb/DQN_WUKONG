...
def random_batch(config: ModelConfig, batch_size: int = 2) -> ReplayBatch:
    ...
    return ReplayBatch(
        ...
        action_masks=np.ones((batch_size, length + 1, ACTION_MASK_SIZE), dtype=np.bool_),
        previous_actions=np.zeros((batch_size, length + 1, 2), dtype=np.int64),
        previous_rewards=np.zeros((batch_size, length + 1), dtype=np.float32),
        actions=np.stack(
            [
                rng.integers(0, MovementToken.size(), (batch_size, length), dtype=np.int64),
                rng.integers(0, CombatToken.size(), (batch_size, length), dtype=np.int64),
            ],
            axis=-1,
        ),
...
def test_visual_encoder_receives_gradient_and_checkpoint_roundtrips(tmp_path) -> None:
    ...
    restored = R2D3Agent(len(HUD_KEYS), ACTION_MASK_SIZE, config, device="cpu")
...
def test_learner_canonicalizes_masked_demonstration_actions() -> None:
    ...
    batch.actions[1, index, 1] = int(CombatToken.SKILL_1)
    batch.action_masks[1, index, COMBAT_MASK_SLICE.start + int(CombatToken.SKILL_1)] = False
    agent = R2D3Agent(len(HUD_KEYS), ACTION_MASK_SIZE, config, device="cpu")
